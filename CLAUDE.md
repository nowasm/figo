...
（figo2godot / figo2cocos / figo2unity）与 web2canvas 的**源码**已拆分到同级
**figo-convert** 仓库——它 add_subdirectory 本仓库拿核心库，改导出逻辑去那边。
但**预编译导出器随本仓库附带**（`prebuild/win-x64/`、`prebuild/macos/`），
普通"设计稿 → 预制体"转换任务直接用它们即可（用法见
`skills/export-prefab/SKILL.md`），无需 figo-convert 检出。
