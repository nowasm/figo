...
cd /d D:\work_open\figmalib
if not exist build_web\build.ninja (
    rem Reuse the desktop build's fetched sources when present (offline-safe).
    set EXTRA=
    if exist build\_deps\raylib-src\CMakeLists.txt set EXTRA=!EXTRA! -DFETCHCONTENT_SOURCE_DIR_RAYLIB=D:/work_open/figmalib/build/_deps/raylib-src
    if exist build\_deps\quickjs-src\CMakeLists.txt set EXTRA=!EXTRA! -DFETCHCONTENT_SOURCE_DIR_QUICKJS=D:/work_open/figmalib/build/_deps/quickjs-src
