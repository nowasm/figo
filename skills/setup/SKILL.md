...
三引擎导出器(figo2cocos / figo2unity / figo2godot)与 web2canvas 已拆分到
**figo-convert** 仓库(https://github.com/nowasm/figo-convert),预编译包如仍
附带这些 exe 可直接用;源码构建见该仓库 README。
...
引擎导出器要另 clone figo-convert(与 figo 同级)并在那边构建:
...
属环境问题,可忽略)。BIN 里如有 figo2cocos(引擎导出链,figo-convert 提供)可再做
一次真转换冒烟:

```
"${CLAUDE_PLUGIN_DATA}/bin/figo2cocos" "${CLAUDE_PLUGIN_ROOT}/examples/assets/sample_ui.json" <临时目录>/cocos_out
```
