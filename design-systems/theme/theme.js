// figo theme library — Material You dynamic color for figo apps.
//
// Bundles @material/material-color-utilities (Copyright Google LLC,
// Apache License 2.0 — see design-systems/LICENSE-APACHE-2.0.txt) compiled
// to a single classic script, plus the figoTheme wrapper at the bottom.
// Regenerate the bundle with:
//   npm i @material/material-color-utilities && npx esbuild entry.js \
//     --bundle --format=iife --global-name=MCU --outfile=mcu.js
//   where entry.js is: export * from '@material/material-color-utilities';
var MCU = (() => {
  ...
  __export(entry_exports, {
    ...
    extendSpecVersion: () => extendSpecVersion,
    ...
    sourceColorFromImageBytes: () => sourceColorFromImageBytes,
...
// ---- figoTheme wrapper ------------------------------------------------------
// Turns a seed color into a full Material 3 role palette (light + dark) and
// lands it in the document's theme variables. Roles are kebab-case token
// names ("primary", "on-primary", "surface-container-high", ...) — bind
// component fills to them via {colorVar} / ui.bindFill, then:
//
//   figoTheme.apply("#0b57d0");        // generate + set both modes
//   ui.setThemeMode("dark");           // instant dark mode
//   figoTheme.apply("#9a25ae");        // instant rebrand, same bindings
//
// figoTheme.fromSeed(hex, opts) returns {light: {...}, dark: {...}} without
// touching the document. opts.variant: "tonalSpot" (default) | "vibrant" |
// "expressive" | "neutral" | "monochrome" | "fidelity" | "content" |
// "rainbow" | "fruitSalad"; opts.contrast: -1..1 (0 default).
(function () {
    var SCHEMES = {
        tonalSpot: MCU.SchemeTonalSpot,
        vibrant: MCU.SchemeVibrant,
        expressive: MCU.SchemeExpressive,
        neutral: MCU.SchemeNeutral,
        monochrome: MCU.SchemeMonochrome,
        fidelity: MCU.SchemeFidelity,
        content: MCU.SchemeContent,
        rainbow: MCU.SchemeRainbow,
        fruitSalad: MCU.SchemeFruitSalad,
    };
    var kebab = function (s) {
        return s.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase();
    };
    // Fixed semantic extensions (Sober parity): success/warning do NOT follow
    // the seed — alerts must stay green/amber whatever the brand color is.
    var EXTRA = {
        light: {
            "success": "#006d43", "on-success": "#ffffff",
            "success-container": "#92f7bc", "on-success-container": "#002111",
            "warning": "#6f5d00", "on-warning": "#ffffff",
            "warning-container": "#ffe169", "on-warning-container": "#221b00",
        },
        dark: {
            "success": "#76daa1", "on-success": "#003920",
            "success-container": "#005231", "on-success-container": "#92f7bc",
            "warning": "#e2c54b", "on-warning": "#3a3000",
            "warning-container": "#544600", "on-warning-container": "#ffe169",
        },
    };
    var roleNames = null;
    var mdc = null;
    var rolesOf = function (scheme) {
        if (!mdc) {
            mdc = new MCU.MaterialDynamicColors();
            roleNames = [];
            var proto = Object.getPrototypeOf(mdc);
            var names = Object.getOwnPropertyNames(proto);
            for (var i = 0; i < names.length; i++) {
                var k = names[i];
                if (k === "constructor" || /PaletteKeyColor$/.test(k)) continue;
                try {
                    var dc = mdc[k]();
                    if (dc && typeof dc.getArgb === "function") roleNames.push(k);
                } catch (e) {}
            }
        }
        var out = {};
        for (var j = 0; j < roleNames.length; j++) {
            ...
            out[kebab(name)] = MCU.hexFromArgb(mdc[name]().getArgb(scheme));
        }
        return out;
    };
    globalThis.figoTheme = {
        fromSeed: function (hex, opts) {
            opts = opts || {};
            var Scheme = SCHEMES[opts.variant || "tonalSpot"];
            if (!Scheme) throw new Error("unknown theme variant '" + opts.variant + "'");
            var hct = MCU.Hct.fromInt(MCU.argbFromHex(hex));
            var contrast = opts.contrast || 0;
            var light = rolesOf(new Scheme(hct, false, contrast));
            var dark = rolesOf(new Scheme(hct, true, contrast));
            for (var k in EXTRA.light) light[k] = EXTRA.light[k];
            for (var k2 in EXTRA.dark) dark[k2] = EXTRA.dark[k2];
            return { light: light, dark: dark };
        },
        apply: function (hex, opts) {
            var t = this.fromSeed(hex, opts);
            ui.setVariables(t.light, "light");
            ui.setVariables(t.dark, "dark");
            return t;
        },
    };
})();
