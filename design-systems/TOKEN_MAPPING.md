# design-tokens.json → figoedit 映射

`<system>/design-tokens.json` 是 W3C Design Tokens 风格的 token 表，每条形如
`{name, value, type}`。本文件规定 AI 怎么把这些 token 折算成 figoedit MCP 的
`create_node` / `update_nodes` 属性。**这是上游"产 HTML/CSS"和我们"建 Figma 节点"
之间的唯一翻译层。**

figoedit 属性以 `apps/editor/editor_mcp.cpp` 的工具 schema 为准（见 create_node /
update_nodes 描述）。颜色一律 `#RRGGBB` 或 `#RRGGBBAA`。

## 按 token type 映射

| token `type` | 例子 | figoedit 落点 |
|---|---|---|
...
| `number` | `--leading-tight = 1.00` | 行高倍数 → `lineHeight`(px) = `round(倍数 × fontSize)`。figoedit 的 `lineHeight` 是**像素**，不是倍数 |
...
- `--tracking-*`（em）→ `letterSpacing`(px) = `em × fontSize`（figoedit `letterSpacing` 是像素）

## CSS-ism 折算坑（必看）

上游 token 值带 CSS 语法，**不能原样塞进 figoedit**，按下表先归一化：
...
| `9999px` / pill 圆角 | figoedit `cornerRadius` 无"全圆"语义 → 设为 `cornerRadius = 节点高度 / 2` |
...
BACKGROUND_BLUR（毛玻璃）**已支持**（CPU 渲染路径；GL 直渲下退化为半透明面板）：背景模糊类
token 折算为 `effects[{type:"BACKGROUND_BLUR", radius}]` + 半透明 fill，模糊每帧重采样、
滚动内容下保持"活"的。
...
3. 按本文件把值折算成 figoedit 属性，`create_node` / `update_nodes` 建设计。
4. `figoplay <app> --shot` 截图，对照 `skills/design-review` 自评后迭代。
