...
std::unique_ptr<Node> parseCanvasNode(const json& j, Node* parent, int depth) {
    ...
    // Mask flag: REST-style "isMask", or the .fig kiwi field "mask".
    node->isMask = jbool(j, "isMask", jbool(j, "mask", false));
    if (const std::string bm = jstr(j, "blendMode");
        !bm.empty() && bm != "NORMAL" && bm != "PASS_THROUGH") {
        node->blendMode = bm;
    }
    ...
    if (node->type == NodeType::Text) {
        ...
        ts.paragraphSpacing = jfloat(j, "paragraphSpacing", 0);
        ts.tabularFigures = jbool(j, "tabularFigures", false);
