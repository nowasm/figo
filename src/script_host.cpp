...
JSValue ui_setVariant(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    ...
    // optional 4th arg: {duration, animate: "dissolve"|"smart"}
    double dur = 0;
    FigmaUI::VariantAnim anim = FigmaUI::VariantAnim::Dissolve;
    if (argc >= 4 && JS_IsObject(argv[3])) {
        JSValue v = JS_GetPropertyStr(ctx, argv[3], "duration");
        if (JS_IsNumber(v)) JS_ToFloat64(ctx, &dur, v);
        JS_FreeValue(ctx, v);
        JSValue av = JS_GetPropertyStr(ctx, argv[3], "animate");
        std::string mode;
        if (JS_IsString(av) && argName(ctx, av, mode) && mode == "smart") {
            anim = FigmaUI::VariantAnim::Smart;
        }
        JS_FreeValue(ctx, av);
    }
    return JS_NewBool(ctx, im->ui.setVariant(name, prop, value,
                                             static_cast<float>(dur < 0 ? 0 : dur), anim));
}
...
JSValue ui_setThemeMode(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    std::string mode;
    if (argc < 1 || !argName(ctx, argv[0], mode)) return JS_EXCEPTION;
    return JS_NewBool(ctx, im->ui.setThemeMode(mode));
}

JSValue ui_themeMode(JSContext* ctx, JSValueConst, int, JSValueConst*) {
    return JS_NewString(ctx, ScriptHost::Impl::from(ctx)->ui.themeMode().c_str());
}

JSValue ui_themeModes(JSContext* ctx, JSValueConst, int, JSValueConst*) {
    auto* im = ScriptHost::Impl::from(ctx);
    JSValue arr = JS_NewArray(ctx);
    uint32_t i = 0;
    for (const auto& m : im->ui.themeModes()) {
        JS_SetPropertyUint32(ctx, arr, i++, JS_NewString(ctx, m.c_str()));
    }
    return arr;
}

// ui.setVariable(name, "#RRGGBB[AA]" | number, mode?) — single token. A
// string value sets a color variable, a number sets a number variable
// (separate namespaces).
JSValue ui_setVariable(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    std::string name, mode;
    if (argc < 2 || !argName(ctx, argv[0], name)) return JS_EXCEPTION;
    if (argc >= 3 && !JS_IsUndefined(argv[2]) && !argName(ctx, argv[2], mode)) {
        return JS_EXCEPTION;
    }
    if (JS_IsNumber(argv[1])) {
        double v = 0;
        if (JS_ToFloat64(ctx, &v, argv[1])) return JS_EXCEPTION;
        im->ui.setVariable(name, static_cast<float>(v), mode);
        return JS_NewBool(ctx, true);
    }
    std::string hex;
    if (!argName(ctx, argv[1], hex)) return JS_EXCEPTION;
    Color c;
    if (!colorFromHex(hex, c)) return JS_NewBool(ctx, false);
    im->ui.setVariable(name, c, mode);
    return JS_NewBool(ctx, true);
}

// ui.setVariables({name: "#hex", ...}, mode?) — batch (one resolve pass at the
// end would be nicer, but per-set resolve is cheap at app scale).
JSValue ui_setVariables(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    if (argc < 1 || !JS_IsObject(argv[0])) {
        return JS_ThrowTypeError(ctx, "object of {name: \"#hex\"} expected");
    }
    std::string mode;
    if (argc >= 2 && !JS_IsUndefined(argv[1]) && !argName(ctx, argv[1], mode)) {
        return JS_EXCEPTION;
    }
    ...
    for (uint32_t i = 0; i < count; ++i) {
        ...
        Color c;
        double num = 0;
        if (key && JS_IsNumber(v) && !JS_ToFloat64(ctx, &num, v)) {
            im->ui.setVariable(key, static_cast<float>(num), mode);
        } else if (key && argName(ctx, v, hex) && colorFromHex(hex, c)) {
            im->ui.setVariable(key, c, mode);
        } else {
            allOk = false;
        }
        JS_FreeValue(ctx, v);
    ...
    js_free(ctx, props);
    return JS_NewBool(ctx, allOk);
}

JSValue ui_getVariable(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    std::string name, mode;
    if (argc < 1 || !argName(ctx, argv[0], name)) return JS_EXCEPTION;
    if (argc >= 2 && !JS_IsUndefined(argv[1]) && !argName(ctx, argv[1], mode)) {
    ...
    Color c;
    if (im->ui.getVariable(name, c, mode)) return JS_NewString(ctx, colorToHex(c).c_str());
    float v = 0;
    if (im->ui.getVariable(name, v, mode)) return JS_NewFloat64(ctx, v);
    return JS_NULL;
}

// ui.bindFill(nameOrNode, varName) — bind the node's first fill to a token.
JSValue ui_bindFill(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    if (argc < 2) return JS_ThrowTypeError(ctx, "node/name and variable expected");
    ...
    std::string varName;
    if (!argName(ctx, argv[1], varName)) return JS_EXCEPTION;
    if (!n) return JS_NewBool(ctx, false);
    return JS_NewBool(ctx, im->ui.bindFillVariable(*n, varName));
}

// ui.bindVar(nameOrNode, prop, varName) — bind a numeric node property to a
// number token. Empty varName clears the binding.
JSValue ui_bindVar(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    if (argc < 3) return JS_ThrowTypeError(ctx, "node/name, property and variable expected");
    Node* n = JS_IsObject(argv[0]) ? nodeOf(ctx, argv[0]) : nullptr;
    if (!n) {
        std::string name;
        if (!argName(ctx, argv[0], name)) return JS_EXCEPTION;
    ...
    std::string prop, varName;
    if (!argName(ctx, argv[1], prop) || !argName(ctx, argv[2], varName)) return JS_EXCEPTION;
    if (!n) return JS_NewBool(ctx, false);
    return JS_NewBool(ctx, im->ui.bindNumberVariable(*n, prop, varName));
}
...
// ui.diagnostics(opts?) — opts {textStress, touchTargets, states} enables the
// opt-in checks; no argument keeps the base contract (empty array = clean).
JSValue ui_diagnostics(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    auto* im = ScriptHost::Impl::from(ctx);
    DiagnosticOptions opts;
    if (argc >= 1 && JS_IsObject(argv[0])) {
        JSValue v = JS_GetPropertyStr(ctx, argv[0], "textStress");
        double d = 0;
        if (JS_IsNumber(v) && !JS_ToFloat64(ctx, &d, v)) opts.textStress = static_cast<float>(d);
        JS_FreeValue(ctx, v);
        v = JS_GetPropertyStr(ctx, argv[0], "touchTargets");
        opts.touchTargets = JS_ToBool(ctx, v) > 0;
        JS_FreeValue(ctx, v);
        v = JS_GetPropertyStr(ctx, argv[0], "states");
        opts.states = JS_ToBool(ctx, v) > 0;
        JS_FreeValue(ctx, v);
    }
    JSValue arr = JS_NewArray(ctx);
    uint32_t i = 0;
    for (const auto& d : im->ui.diagnostics(opts)) {
...
ScriptHost::ScriptHost(FigmaUI& ui) : impl_(std::make_unique<Impl>(ui)) {
    ...
    fn("setThemeMode", ui_setThemeMode, 1);
    fn("themeMode", ui_themeMode, 0);
    fn("themeModes", ui_themeModes, 0);
    fn("setVariable", ui_setVariable, 3);
    fn("setVariables", ui_setVariables, 2);
    fn("getVariable", ui_getVariable, 2);
    fn("bindFill", ui_bindFill, 2);
    fn("bindVar", ui_bindVar, 3);
    fn("find", ui_find, 1);
    fn("findAll", ui_findAll, 1);
    fn("diagnostics", ui_diagnostics, 1);
