...
把 **Figma 设计稿（以及 React/HTML 页面）直接作为游戏 UI** 渲染到游戏引擎中的
C++ 矢量 UI 库。
...
所有输入都归一到同一个中间表示 **canvas.json**，再由核心库渲染或导出：

```
输入                                   中间表示              输出
.fig 文件 ──fig2json──────────┐
Figma REST JSON (?geometry=paths) ─┤── canvas.json ─▶ figo::Document ─▶ ThorVG 光栅化 ─▶ 引擎纹理
React/HTML ──web2canvas───────┘   （自动识别格式）   (Frame/Text/…)      （raylib / Unity / 自研运行时…）
       （真实浏览器渲染后采集）
```
...
```
节点树  figo::Document (Frame / Rect / Ellipse / Vector / Text / ...)
        │  scene_builder
        ▼
...
用 figo 最快的方式是通过 [Claude Code](https://claude.com/claude-code) 插件——
它把完整的 AI 闭环(脚手架 → figoedit MCP 设计 → 写逻辑 → 截图自验 →
导出 Cocos/Unity/Godot)打包成技能:
...
装完后让 Claude 跑一次 **`figo:setup`** 技能——它会从 GitHub Releases 下载
Windows 预编译二进制(macOS/Linux 走源码构建,技能里有完整步骤)并验证工具链。

| 技能 | 用途 |
|---|---|
| `figo:setup` | 安装/修复工具链(先跑这个) |
...
```
include/figo/   公共 API（document / parser / renderer / ui / script）
src/                解析器、SVG path 解析、ThorVG 场景构建、字体、渲染器、脚本宿主
backends/raylib/    raylib 后端（其他引擎后端的参考模板，~200 行）
apps/editor/        figoedit — Figma 风格的可视化编辑器（raylib + raygui + MCP）
apps/figoplay/     figoplay — 通用脚本播放器（app = .fig + .js，热重载）
examples/           demo_raylib / demo_wallet 演示程序、示例脚本与设计文件
  assets/           测试/演示用的 Figma JSON 与 .fig
  scripts/          figoplay 示例脚本（wallet.js）
...
```
build\figoedit.exe [design.fig | canvas.json | file.json]   # 或拖放文件到窗口
build\figoedit.exe --selftest [file]                        # 无窗口逻辑自测
```
...
保存写出 `<原文件>.figo.json`（REST 格式，可直接被 figo 重新加载，
不覆盖原 .fig）。HiDPI 缩放自动检测，可用 `FIGOEDIT_SCALE` 环境变量覆盖。

### MCP 服务器（AI 直接在编辑器里设计）

figoedit 启动时内嵌一个 MCP 服务器（Streamable HTTP，仅监听本机
`127.0.0.1:9223`，端点 `/mcp`），AI 客户端连上后即可读取图层树、增删改节点、
截图自检，全部修改与用户共享同一条 undo 历史（Ctrl+Z 可撤销 AI 的任何编辑）。

```
build\figoedit.exe --mcp-port 9300 design.fig    # 改端口
build\figoedit.exe --no-mcp design.fig           # 关闭 MCP
set FIGOEDIT_MCP_PORT=9300                       # 环境变量同效
```
...
2. 构建 figo（raylib 由 CMake FetchContent 自动拉取）：
...
不写 C++ 也能做 app：**设计是 .fig，逻辑是 .js**。`figo_script` 把
FigmaUI API 绑定进 QuickJS（quickjs-ng，CMake 自动拉取），`figoplay`
是通用播放器：
...
```js
// wallet.js — 完整 API 见 include/figo/script.h
...
字体、设计系统、打包元数据收拢成一个工程，直接传目录给 figoplay：

```
figoplay examples/apps/sample            # 读 sample/app.json
```
...
// examples/apps/sample/app.json
{
  ...
  "package": { "id": "com.figo.sample", "version": "1.0.0" }  // 预留给打包
}
```
...
figo **进程内**完成 .fig→JSON 转换（[fig2json](https://github.com/kreako/fig2json)
编译为静态库直接链接，CMake `FIGO_FIG2JSON_LIB` 指定 .lib 路径，
`cargo build --release` 产出），无外部进程依赖；结果缓存在
`design.fig.export/`（.fig 未变化时复用），图片自动从 .fig 内嵌数据解出并
接入渲染。静态库缺席时回退调用 fig2json CLI（查找顺序：`FIGO_FIG2JSON`
环境变量 → CMake `FIGO_FIG2JSON` 默认路径 → PATH）。也可以手动预转换
...
- 蒙版图层（`isMask`：该节点的 alpha 蒙版其上方同级，自身不绘制；支持形状/
  渐变 alpha 淡出蒙版，多枚蒙版按 Figma 语义嵌套）、图层混合模式（`blendMode`：
  MULTIPLY/SCREEN/OVERLAY/…全套映射 ThorVG BlendMethod）
- 效果：投影（DROP_SHADOW）、内阴影（INNER_SHADOW，反形模糊+遮罩仿真，
  spread 不支持）、图层模糊（LAYER_BLUR）、背景模糊（BACKGROUND_BLUR 毛玻璃，
  双 pass 采样下层内容烘焙模糊，滚动下每帧保鲜；CPU 路径）
...
- 字体约定：输入文件旁的 `fonts/` 目录与 `FIGO_FONTS_DIR` 环境变量自动注册
...
- **响应式布局**：约束（LEFT/RIGHT/CENTER/STRETCH/SCALE 双轴）+ 自动布局
  ...
  ```cpp
  ui->setResizeMode(figo::FigmaUI::ResizeMode::Reflow);  // 默认 Scale
  ui->setViewport(w, h);   // 触发 layoutFrame() 重排（demo 中按 R 切换）
  ```

  重排基于解析时快照的原始几何（`Node::base*`），反复 resize 不累积误差；
  布局引擎也可单独使用：`figo::layoutFrame(frame, w, h)`。
...
- **win**：`figoplay.exe` + `app/` + `run.cmd`（双击即跑）
...
- `icon`（方形 PNG）→ web favicon + apple-touch-icon、android 各密度
  `mipmap/ic_launcher`、**win 把图标内嵌进 exe**（用 .rc 重链 figoplay，需要
...
tools\build_web.cmd            # 产出 build_web\figoplay.{html,js,wasm,data}
python -m http.server 8123 -d build_web   # 打开 http://localhost:8123/figoplay.html
```
...
NativeActivity（`hasCode=false`）直接加载 libfigoplay.so，aapt 手工打包：

```
tools\build_thorvg_android.cmd    # ThorVG arm64-v8a + x86_64 静态库
powershell tools\build_android.ps1  # NDK 双 ABI 编译 → build_android\figoplay.apk
...
// GPU：当前 GL 上下文下，把 FBO id 交给 figo
...
- 布局期文本测量已支持 autoResize=HEIGHT 的自动高度（框高随排版结果走，auto-layout
  链内生效）；**平面父级下的裸 TEXT 在 authored 尺寸下不重排**；BASELINE 对齐仍用
  字体样式近似基线（非真实字体度量）
- 原型交互（prototype interactions）；库组件变体（组件集不在文档内时）
- BACKGROUND_BLUR 仅 CPU 渲染路径（GL 直渲/renderOverlay 转场贴图不采样，退化为
  半透明面板）；INNER_SHADOW 的 spread 参数；
  描边上的角度/菱形渐变（退化为径向）
