...
React/HTML ──web2canvas───────┤── canvas.json ─▶ figo::Document ─┬─▶ ThorVG 光栅化 ─▶ 引擎纹理
       （真实浏览器渲染后采集）  │   （自动识别格式）   (Frame/Text/…)   │    （raylib / Unity / 自研运行时…）
                                                                      └─▶ figo2godot ─▶ Godot 4 工程
                                                                              (.tscn + PNG sprites)
```
...
apps/figo2godot/    figo2godot — canvas.json → Godot 4 工程（.tscn + sprites）
...
tools/              figmanew / figmapack / gen_templates（Python）、web2canvas（Node）
  web2canvas/       React/HTML → canvas.json（Playwright 驱动真实浏览器）
...
## React/HTML → Godot 工程（web2canvas + figo2godot）

除了 Figma，**一个 React/HTML 页面也能直接变成可在 Godot 4 打开的工程**。页面在
真实无头浏览器里渲染后采集，所以 flex/grid 布局、`oklch` 颜色、`clip-path` 切角、
渐变、阴影、字体宽度都按浏览器算好的结果来；flat fill 表达不了的（渐变 / 图片 /
内联 `<svg>` / clip-path / 旋转元素 / 虚线边框 / 外发光）会光栅化成 PNG `IMAGE`
fill，与"矢量 → 贴图"目标一致。整条链已在真实页面（GOGO KILL HUD，8 屏）端到端
验证。
...
把渲染后的 DOM 采集成 canvas.json。驱动**已安装的 Edge/Chrome**（playwright-core，
不下载 Chromium），内置 react / react-dom / @babel/standalone（Babel-in-browser 的
非 ESM React 应用也能加载；CDN 请求由内置副本拦截应答，本地文件经临时 HTTP 服务
托管以便 Babel 的 XHR 加载 .jsx）。
...
| 参数 | 含义 |
|---|---|
| `--root SEL` | 采集的根元素（如 `#stage`）；默认 `body` |
| `--viewport WxH` | 浏览器视口；对准设计舞台尺寸才 1:1 |
| `--states "a,b,c"` | 多屏：逐个调用 `window.<navFn>(state)`，每屏一个顶层 frame |
| `--nav-fn FN` | 切屏的全局函数名（默认 `__nav`） |
| `--fonts DIR` | 注入项目的 `fonts.css`，让文本按真实字体宽度测量 |

### figo2godot（`apps/figo2godot/`，C++）

把 canvas.json 转成 Godot 4 工程：每个顶层 frame → 一个 `.tscn`，去重的 PNG
sprites（内容哈希去重）、绑定字体、`manifest.json`、`project.godot`。节点映射：
TEXT→Label、纯色矩形/容器→ColorRect、圆角面板→NinePatchRect+sprite、
椭圆/矢量/渐变/图片/描边/效果→TextureRect+烘焙 sprite（sprite 由
`Renderer::renderOverlay` 渲染，与运行时像素一致）。响应式约束 → Godot anchor/offset。
该工具只链接 figo 核心静态库（不需要 raylib/quickjs），可在精简的
`build_godot` 目录里秒级编出。
...
`--prefabs`：把重复出现的组件（卡片/按钮/行）抽成 `components/*.tscn`（PackedScene），
每处实例化并按实例覆盖文本——真正的 prefab 复用而非 inline 复制。
...
跑通 web2canvas → figo2godot，中间产物落在 `<godotDir>/.web2canvas/`，`<godotDir>/`
即可直接用 Godot 4 打开。完整示例（GOGO KILL HUD，8 屏 → 8 个场景）：
...
> AI 工作流：`web-to-godot` skill（`.claude/skills/web-to-godot/`）把这条链包成
> 端到端流程（装环境 → 一条命令 → Godot 打开截图自验 → 迭代）。

**已知限制**：只采 screen 级（`window.__nav`）——仅靠点击打开的弹窗 / overlay、
每屏滚动超出视口的内容暂不捕获；letter-spacing（Godot Label 无原生支持）、每角
独立圆角的 NinePatch（取最大角）。Google Fonts 在采集时被掐，用 `--fonts` 指向
本地字体。
