...
if (globalThis.SELFDRIVE) {
    ...
    ui.onUpdate(() => {
        ...
        // Smart animate: matched children tween geometry (Bar width 40->120)
        // instead of the root dissolving; the On-only Badge fades in. Runs
        // alongside the Fade Button dissolve (different instances) to fit
        // inside figoplay's 140-frame selfdrive budget.
        if (frames === 90) {
            ui.setVariant("Smart Button", "Mode", "On",
                          { duration: 0.4, animate: "smart" });
            const bar = ui.find("Smart Button").find("Bar");
            check("smart from state", near(bar.width, 40, 1), "true");
            check("smart no root fade",
                  near(ui.find("Smart Button").opacity, 1, 0.01), "true");
        }
        if (frames === 94) {  // a few ticks in: strictly mid-tween
            const w = ui.find("Smart Button").find("Bar").width;
            check("smart mid tween", w > 44 && w < 118, "true");
            check("smart root stays opaque",
                  near(ui.find("Smart Button").opacity, 1, 0.01), "true");
        }
        if (frames === 132) {  // ~0.7s in: settled on the target variant
            const btn = ui.find("Smart Button");
            check("smart done", near(btn.find("Bar").width, 120, 0.1), "true");
            check("smart badge appeared", btn.find("Badge") !== null, "true");
            check("smart badge sized",
                  btn.find("Badge") ? near(btn.find("Badge").width, 12, 0.1) : false,
                  "true");
        }
